...
def compare_txt(pattern,testname):
    ...
    t1 = data1[:,-1]
    t2 = data2[:,-1]
    print(  'TIME ORGINAL\t\taverage: {0:.5f}'.format(np.mean(t1[1:])),'\tstd.dev.: {0:.5f}'.format(np.std(t1[1:])) )
    print('TIME DEVELOPMENT\taverage: {0:.5f}'.format(np.mean(t2[1:])),'\tstd.dev.: {0:.5f}'.format(np.std(t2[1:])) )
    print()
